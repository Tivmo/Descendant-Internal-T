...
static __int64 YourHookProc(void* self, void* Canvas)
{
	if (Canvas)
	{
		...
		if (CheckPointers())
		{
			...
			if (cfg_GrappleAbility)
			{
				ModifyGrappleAbility();
			}
...
void ModifyGrappleAbility()
{
	...
	static bool foundGrappleAbility = false;
	static TFD_SDK::UM1WireSkillAbility* GrappleAbility = nullptr;

	if (!foundGrappleAbility)
	{
		for (int i = 0; i < TFD_SDK::UObject::GObjects->Num(); i++)
		{
			TFD_SDK::UObject* Obj = TFD_SDK::UObject::GObjects->GetByIndex(i);
			...
			if (Obj->IsA(TFD_SDK::UM1WireSkillAbility::StaticClass()))
			{
				TFD_SDK::UM1WireSkillAbility* Ability = static_cast<TFD_SDK::UM1WireSkillAbility*>(Obj);
				if (!Ability)
					continue;

				GrappleAbility = Ability;
				foundGrappleAbility = true;
				break;
			}
		}
	}
	else
	{
		if (!GrappleAbility)
		{
			foundGrappleAbility = false;
			GrappleAbility = nullptr;
			return;
		}
		else
		{
			if (GrappleAbility->Flags & TFD_SDK::EObjectFlags::BeginDestroyed ||
				GrappleAbility->Flags & TFD_SDK::EObjectFlags::BeingRegenerated ||
				GrappleAbility->Flags & TFD_SDK::EObjectFlags::FinishDestroyed ||
				GrappleAbility->Flags & TFD_SDK::EObjectFlags::NeedInitialization ||
				GrappleAbility->Flags & TFD_SDK::EObjectFlags::WillBeLoaded)
			{
				foundGrappleAbility = false;
				GrappleAbility = nullptr;
				return;
			...
			GrappleAbility->MaxRange = 1000000.0f; // Infinite range
			GrappleAbility->CooldownTime = 0.1f;   // Fast recharge
		}
	}
}
...
void DrawMenu()
{
	if (ZeroGUI::Window((char*)"DescendantInternal - UnknownCheats.me", &cfg_WindowPos, TFD_SDK::FVector2D{ 500.0f, 600.0f }, cfg_DrawMenu))
	{
		...
		if (tab == 2)
		{
			...
			ZeroGUI::Checkbox((char*)"Enable Grapple Ability", &cfg_GrappleAbility);
		}
		if (tab == 3)
		{
...
void LoadCFG()
{
	...
	else
	{
		...
		cfg_GrappleAbility = ini.GetBoolValue("Extra", "EnableGrappleAbility");
...
void SaveCFG()
{
	...
	ini.SetBoolValue("Extra", "EnableGrappleAbility", cfg_GrappleAbility);
