			...
			for (int i = 0; i < Actors->Characters.Num(); i++)
			{
				...
				if (p)
				{
						...
						if (WorldToScreen(p->K2_GetActorLocation(), &ScreenPos))
						{
							if (cfg_DrawPlayerNames)
							{
								if (player->PlayerName && player->PlayerName.ToString() != "")
								{
									...
									ZeroGUI::TextCenter((char*)Name.c_str(), TFD_SDK::FVector2D{ ScreenPos.X, ScreenPos.Y }, ZeroGUI::Colors::Text, false);
								}
							}
							if (cfg_DrawPlayerBoxes)
							{
								...
								if (cfg_DrawCirclesInstead)
								{
									...
									ZeroGUI::DrawCircle(TFD_SDK::FVector2D{ ScreenPos.X, ScreenPos.Y }, AdjustedRadius, 64, FriendColor);
								}
								else
								{
									...
									ZeroGUI::DrawRectangle(TFD_SDK::FVector2D{ ScreenPos.X, ScreenPos.Y }, TFD_SDK::FVector2D{ BoxWidth, BoxHeight }, FriendColor);
								}
							}
							if (cfg_DrawPlayerLines)
								ZeroGUI::DrawActorLine(TFD_SDK::FVector2D{ ScreenPos.X, ScreenPos.Y }, FriendColor);
						}
					...
					else if (p->IsA(TFD_SDK::AM1Monster::StaticClass()) || p->CharacterAttribute->IsA(TFD_SDK::UM1MonsterAttribute::StaticClass()))
					{
						...
						if (WorldToScreen(p->K2_GetActorLocation(), &ScreenPos))
						{
							TFD_SDK::FLinearColor color = EnemyColor;
							...
							if (cfg_DrawEnemyNames)
							{
								...
								{
									ZeroGUI::TextCenter((char*)IDNameMap[(int)p->CharacterId.ID].c_str(), TFD_SDK::FVector2D{ ScreenPos.X, ScreenPos.Y }, ZeroGUI::Colors::Text, false);
								}
							}
							if (cfg_DrawEnemyBoxes)
							{
								...
								if (cfg_DrawCirclesInstead)
								{
									float AdjustedRadius = (cfg_ESPBox.X / ODistance) * cfg_ScaleFactor;
									AdjustedRadius = (AdjustedRadius < 3.0f) ? 3.0f : (AdjustedRadius > 70.0f) ? 70.0f : AdjustedRadius;
									ZeroGUI::DrawCircle(TFD_SDK::FVector2D{ ScreenPos.X, ScreenPos.Y }, AdjustedRadius, 64, EnemyColor);
								}
								else
								{
									float BoxWidth = (cfg_ESPBox.X / ODistance) * cfg_ScaleFactor;
									...
									BoxWidth = (BoxWidth < 2.0f) ? 2.0f : (BoxWidth > 65.0f) ? 65.0f : BoxWidth;
									BoxHeight = (BoxHeight < 4.0f) ? 4.0f : (BoxHeight > 90.0f) ? 90.0f : BoxHeight;
									ZeroGUI::DrawRectangle(TFD_SDK::FVector2D{ ScreenPos.X, ScreenPos.Y }, TFD_SDK::FVector2D{ BoxWidth, BoxHeight }, EnemyColor);
								}
							}
...
void DrawMenu()
{
	if (ZeroGUI::Window((char*)"Descendant Internal - Tivmo Edition", &cfg_WindowPos, TFD_SDK::FVector2D{ menuWidth, menuHeight }, cfg_DrawMenu))
	{
		...
		if (ZeroGUI::ButtonTab((char*)"Info", TFD_SDK::FVector2D{ 70, 15 }, tab == 5)) tab = 5;
		...
		if (tab == 3)
		{
			...
			ZeroGUI::SliderFloat((char*)"Timescale:", &cfg_TimeScale, 0.25f, 3.0f);
		...
		if (tab == 4)
		{
			...
			ZeroGUI::ColorPicker((char*)"Friend Color", &FriendColor);
			ZeroGUI::Text((char*)"Check Extras Tab for more Customization info.");
		}
		if (tab == 5)
		{
			ZeroGUI::Text((char*)"Extra Info");
			ZeroGUI::Text((char*)"Use Instant Infiltration after starting one");
			ZeroGUI::Text((char*)"This should skip generators and spawn the boss.");
			ZeroGUI::Text((char*)"Arrow keys can be used to adjust the Menu size");
			ZeroGUI::Text((char*)"Up/Down for Vertical - Left/Right for Width");
			ZeroGUI::Text((char*)"Up/Right are increase, Left/Down are decrease");
			ZeroGUI::Text((char*)"The colors in the color picker are doubled.");
			ZeroGUI::Text((char*)"Left half, first 6 Columns, are Full Alpha.");
			ZeroGUI::Text((char*)"Right half, other 6 Columns, are Half Alpha.");
			ZeroGUI::Text((char*)"Incase having too many ESPs makes seeing hard.");
			ZeroGUI::Text((char*)"Apologies for vanishing, down on luck in life.");
			ZeroGUI::Text((char*)"If requested I will try to maintain my version.");
		}
		...
		if (tab == 4)
		{
			...
			ZeroGUI::ColorPicker((char*)"Enemy Color", &EnemyColor);
		}
		ZeroGUI::Render();
...
void LoadCFG()
{
	...
	else
	{
		...
		cfg_DrawCirclesInstead = ini.GetBoolValue("ESP", "CircleESP");
		...
		FriendColor = TFD_SDK::FLinearColor(
			static_cast<float>(ini.GetDoubleValue("Colors", "HealthColorR", 0.0f)),
			static_cast<float>(ini.GetDoubleValue("Colors", "HealthColorG", 1.0f)),
			static_cast<float>(ini.GetDoubleValue("Colors", "HealthColorB", 0.0f)),
			static_cast<float>(ini.GetDoubleValue("Colors", "HealthColorA", 1.0f)));
		EnemyColor = TFD_SDK::FLinearColor(
			static_cast<float>(ini.GetDoubleValue("Colors", "HealthColorR", 1.0f)),
			static_cast<float>(ini.GetDoubleValue("Colors", "HealthColorG", 0.0f)),
			static_cast<float>(ini.GetDoubleValue("Colors", "HealthColorB", 0.0f)),
			static_cast<float>(ini.GetDoubleValue("Colors", "HealthColorA", 1.0f)));
...
void SaveCFG()
{
	...
	ini.SetBoolValue("ESP", "CircleESP", cfg_DrawCirclesInstead);
	...
	ini.SetDoubleValue("Colors", "FriendColorR", FriendColor.R);
	ini.SetDoubleValue("Colors", "FriendColorG", FriendColor.G);
	ini.SetDoubleValue("Colors", "FriendColorB", FriendColor.B);
	ini.SetDoubleValue("Colors", "FriendColorA", FriendColor.A);

	ini.SetDoubleValue("Colors", "EnemyColorR", EnemyColor.R);
	ini.SetDoubleValue("Colors", "EnemyColorG", EnemyColor.G);
	ini.SetDoubleValue("Colors", "EnemyColorB", EnemyColor.B);
	ini.SetDoubleValue("Colors", "EnemyColorA", EnemyColor.A);
...
DWORD WINAPI Init(HMODULE Module)
{
	...
	if (GameModule.dwBase)
	{
		...
		TFD_SDK::Offsets::GObjects = 0x9E86C50;
#ifdef IS_DEBUG
